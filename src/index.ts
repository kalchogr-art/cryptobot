    ...
    export default {
              async fetch(request: Request, env: Env): Promise<Response> {
                ...
                if (url.pathname === "/forward-dashboard") {
                  ...
                  const netPct=(x:any)=>{const rp=Number(x.realized_pnl),e=Number(x.entry_fill_price),sz=Number(x.entry_fill_size);if(Number.isFinite(rp)&&Number.isFinite(e)&&e>0&&Number.isFinite(sz)&&sz>0)return rp/(e*sz)*100;const g=grossPct(x);return g==null?null:g-0.07;};
                  const stats=(rows:any[],side?:string)=>{
                    ...
                    return {total:r.length,open:r.filter((x:any)=>closeKind(x)==="OPEN").length,closed:closed.length,
                      ...
                      partial:r.filter((x:any)=>Number(x.partial_tp_filled_at??0)>0).length,
                      runnersOpen:r.filter((x:any)=>Number(x.runner_active??0)===1).length,
                      r075:r.filter((x:any)=>Number(x.runner_max_return_pct??0)>=0.75).length,
                      r100:r.filter((x:any)=>Number(x.runner_max_return_pct??0)>=1.00).length,
                      r150:r.filter((x:any)=>Number(x.runner_max_return_pct??0)>=1.50).length,
                      r200:r.filter((x:any)=>Number(x.runner_max_return_pct??0)>=2.00).length,
                      r300:r.filter((x:any)=>Number(x.runner_max_return_pct??0)>=3.00).length,
